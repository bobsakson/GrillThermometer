...
var saveProfile = function(profile) {
    ...
    db.run('INSERT INTO profile(name, description, isDeleted) VALUES($id, $name, $description, $isDeleted)', { $name: profile.name, $description: profile.description, $isDeleted: 0 }, function(err, row) {
        if(err) {
            console.log(err);
        }
        else {
            db.run('SELECT MAX(id) as maxId FROM profile', function(err, row) {
